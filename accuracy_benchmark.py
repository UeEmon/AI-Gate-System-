"""Compare EasyOCR and PaddleOCR on reviewed validation samples."""
import argparse
import json
import os
...
def benchmark(root):
    ...
    samples = [sample for sample in dataset_snapshot(root)
               if sample['partition'] == 'validation']
    if not samples:
        raise ValueError('比較には確認済みvalidationデータが必要です。')
    results = {}
    previous = os.environ.get('GATE_OCR_BACKEND')
    try:
        for backend in ('easyocr', 'paddle'):
            ...
            for sample in samples:
                image = cv2.imdecode(np.frombuffer(sample['image'], np.uint8),
                                     cv2.IMREAD_COLOR)
                for label, (x1, y1, x2, y2) in training_crops(
                        sample, image.shape[1], image.shape[0]):
...
if __name__ == '__main__':
    ...
    print(json.dumps(benchmark(Path(args.root)), ensure_ascii=False, indent=2))
