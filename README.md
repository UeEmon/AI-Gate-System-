...
USB／RTSP／別端末のWebカメラは複数ジョブとして同時処理できます。既定の上限は4台で、
`GATE_MAX_CAMERAS`（1〜16）で変更します。カメラごとにモデルを読み込むため、台数に応じたCPU・
メモリが必要です。同じUSB番号またはRTSP URLの二重起動は拒否し、ファイル処理とは排他実行します。
...
処理履歴画面で「処理履歴」と「認識履歴」を選び、対象件数を確認して一括削除できます。
処理履歴を削除すると、ジョブ情報、アップロードした入力、ログ、プレビュー、ブラウザカメラの一時画像を削除します。
認識履歴を削除すると、観測レコードと保存した車両画像を削除します。削除は元に戻せません。
認識処理の実行中は削除できません。APIは認証・CSRF・確認文字列を検証し、管理データディレクトリ外の
パスを削除しません。登録車両、通知履歴・通知映像、OCR学習データ、学習済みモデルは保持します。
認識履歴を削除した後も学習データで再学習できますが、元の履歴画像を使った正解・範囲の再編集はできません。
SQLiteファイルは行を削除しても直ちに縮小しない場合があります。
...
通常の「処理結果」一覧には、開始時に指定した車両検出・OCR信頼度以上の結果だけを表示します。
既定値は車両80%・OCR70%で、画面から各1〜100%の範囲で処理ごとに変更できます。閾値を下げると
候補数が増える一方で誤検出も増え、上げると確度を優先します。指定値未満は通常結果・連続登録候補
には出さず、OCR不足は「ナンバー要確認」の通知判定に使用します。
車両検出は既定でYOLO入力サイズ960pxを使用し、小さい車両の検出を改善します。OCRは角度補正後、
カラー・コントラスト補正・ノイズ除去・シャープ化・二値化を比較し、同じ番号を複数の前処理で
独立して認識した候補を優先します。処理能力を優先する場合はCLIの `--imgsz` を下げられます。
...
通知一覧は直近10件だけを表示し、その範囲で未確認を先頭にして新しい日付順に並べます。
未確認件数は非表示分を含む全通知から集計します。「未確認をすべて確認済みにする」で、
画面に出ていない通知を含む未確認通知を一括確認できます。
...
利用許諾を確認できない`lprs-jp`は登録から削除しました。
`alpr_jp`はMITの学習素材であり実行モデルではないため、モデル選択から除外しました。
実行アダプターのないモデル群も一覧から除きました。判断理由と残すモデルの条件は
[第三者ライセンス一覧](THIRD_PARTY_NOTICES.md)を参照してください。
...
登録フォームと学習正解フォームは共通です。4項目の登録欄には、選択したOCR候補の検出値を
修正前データとして自動入力します。正しく検出された項目はそのまま残し、誤認識した項目だけを
訂正できます。候補を切り替えると共通フォームの初期値も切り替わります。
...
探索画像の長辺は最大960px、各回の輪郭候補は最大24件とし、四隅が得られない場合だけ
コントラスト補正後に1回再探索します。補正は元画像から行い、OCR用画像には3%の余白を残します。
四隅が得られない輪郭を回転矩形で代用する処理は行いません。
